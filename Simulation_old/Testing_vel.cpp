...
main( int argc, char **argv )
{
  ...
  try
  {
    ...
    float Ts = 0.005; //10ms sampling time
    ...
    std::vector<float> wp_0{-0.4, -0.4, 0.2, 0, 0, 0, 0, 0, 0};
    ...
    std::cout<< " Controller Chosen: Velocity"<<std::endl;
    ...
    while ( (!final_quit))
    {
      ...
      while(i< param)
      {
          // get current time from coppeliasim
        ...
        xd_dot = {velocity(i, 0), velocity(i, 1), velocity(i, 2), 0, 0, 0 };
        ...
        if (i ==0)
          {
            std::cout<<fMed<<std::endl;
          }
        e_x =(vpColVector)vpPoseVector( fMed.inverse() * robot.get_fMe() ); // edMe // pose error [ t, theta * u ] using homogeneous transformations;
        ...
        // apply control law
        qdot = fJe.transpose() * (K_v * e_x + xd_dot) + (I7 - fJe.pseudoInverse()*fJe)*q_0_dot ;
        ...
        plotter->plot( 1, time_cur, qdot );
        plotter->plot( 2, time_cur, e_x );
        pose_err_norm[0] = sqrt( e_x.extract( 0, 3 ).sumSquare() );
        pose_err_norm[1] = sqrt( e_x.extract( 3, 3 ).sumSquare() );
      ...
      // frequency of the loop --> make it possible for the simulator --> 50 ms
      robot.wait( time_cur, Ts); 
