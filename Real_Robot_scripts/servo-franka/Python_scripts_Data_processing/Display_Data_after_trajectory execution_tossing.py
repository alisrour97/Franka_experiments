...
# R_point_pos = np.array([0.287, -0.377, 0.489])
# R_point_vel = np.array([0.7, 0.65, 0.65])

index = 1400
R_point_pos = pos_d[index, :]
R_point_vel = vel_d[index, :]
...
ax1.plot(*pos_d[index, :3], "o", color="b", linewidth=5, markersize=15, alpha=0.4, label='Target')
ax1.set_xlabel('X [m]')
ax1.set_ylabel('Y [m]')
ax1.set_zlabel('Z [m]')
...
print(f"distance error to desired target {np.linalg.norm(pos_d[index, :3] - pos[index, :3])}")
print(f"Velocity error to desired target velocity{np.linalg.norm(vel_d[index, :3] - vel[index, :3])}")
